...
# ═══════════════════════════════════════════════════════════════════
# WORKFLOW ATTRIBUTION (DOTBOT_CURRENT_WORKFLOW)
# ═══════════════════════════════════════════════════════════════════
# The MCP dispatcher injects the current workflow name into task_create /
# task_create_bulk calls so tasks generated by Claude during a workflow run
# can be attributed back to that workflow. The runner sets the env var via
# launch-process.ps1; here we simulate it directly and verify the contract.

Write-Host "  WORKFLOW ATTRIBUTION" -ForegroundColor Cyan
Write-Host "  ────────────────────────────────────────────" -ForegroundColor DarkGray

$wfMcpProcess = $null
$savedWorkflowEnv = $env:DOTBOT_CURRENT_WORKFLOW
try {
    $env:DOTBOT_CURRENT_WORKFLOW = 'test-workflow'
    $wfMcpProcess = Start-McpServer -BotDir $botDir
    $null = Send-McpInitialize -Process $wfMcpProcess
    $wfReqId = 0

    $todoDir = Join-Path $botDir "workspace\tasks\todo"

    # Case A: task_create without explicit workflow → env value is injected
    $wfReqId++
    $resp = Send-McpRequest -Process $wfMcpProcess -Request @{
        jsonrpc = '2.0'
        id      = $wfReqId
        method  = 'tools/call'
        params  = @{
            name      = 'task_create'
            arguments = @{
                name        = 'WF Attribution Implicit'
                description = 'task_create with no explicit workflow arg'
                category    = 'feature'
                effort      = 'S'
            }
        }
    }
    $resultObj = $resp.result.content[0].text | ConvertFrom-Json
    $taskId = $resultObj.task_id
    Assert-True -Name "task_create (implicit workflow) succeeds" `
        -Condition ($resultObj.success -eq $true -and $taskId) `
        -Message "task_create did not succeed"
    if ($taskId) {
        $taskFile = Get-ChildItem -Path $todoDir -Filter "*$($taskId.Split('-')[0])*.json" -ErrorAction SilentlyContinue | Select-Object -First 1
        Assert-True -Name "task_create file written for implicit workflow" `
            -Condition ($null -ne $taskFile) `
            -Message "Task file not found for ID $taskId"
        if ($taskFile) {
            $taskJson = Get-Content $taskFile.FullName -Raw | ConvertFrom-Json
            Assert-Equal -Name "task_create injects DOTBOT_CURRENT_WORKFLOW into workflow field" `
                -Expected 'test-workflow' `
                -Actual $taskJson.workflow
        }
    }

    # Case B: task_create with explicit workflow → explicit value wins
    $wfReqId++
    $resp = Send-McpRequest -Process $wfMcpProcess -Request @{
        jsonrpc = '2.0'
        id      = $wfReqId
        ...
        params  = @{
            name      = 'task_create'
            arguments = @{
                name        = 'WF Attribution Explicit'
                description = 'task_create with explicit workflow arg'
                category    = 'feature'
                effort      = 'S'
                workflow    = 'explicit-caller-wins'
            }
        }
    }
    $resultObj = $resp.result.content[0].text | ConvertFrom-Json
    $taskId = $resultObj.task_id
    Assert-True -Name "task_create (explicit workflow) succeeds" `
        -Condition ($resultObj.success -eq $true -and $taskId) `
        -Message "task_create did not succeed"
    if ($taskId) {
        $taskFile = Get-ChildItem -Path $todoDir -Filter "*$($taskId.Split('-')[0])*.json" -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($taskFile) {
            $taskJson = Get-Content $taskFile.FullName -Raw | ConvertFrom-Json
            Assert-Equal -Name "explicit workflow arg overrides DOTBOT_CURRENT_WORKFLOW" `
                -Expected 'explicit-caller-wins' `
                -Actual $taskJson.workflow
        }
    }

    # Case C: task_create_bulk → each task without explicit workflow gets env value
    $wfReqId++
    $resp = Send-McpRequest -Process $wfMcpProcess -Request @{
        jsonrpc = '2.0'
        id      = $wfReqId
        ...
        params  = @{
            ...
            arguments = @{
                tasks = @(
                    @{
                        name        = 'WF Bulk Implicit A'
                        description = 'no explicit workflow'
                        category    = 'feature'
                        effort      = 'S'
                    },
                    @{
                        name        = 'WF Bulk Explicit B'
                        description = 'has explicit workflow'
                        category    = 'feature'
                        effort      = 'S'
                        workflow    = 'per-task-override'
                    }
                )
            }
        }
    ...
    $resultObj = $resp.result.content[0].text | ConvertFrom-Json
    Assert-True -Name "task_create_bulk succeeds" `
        -Condition ($resultObj.success -eq $true -and $resultObj.created_count -eq 2) `
        -Message "Bulk create did not succeed: $($resp.result.content[0].text)"
    if ($resultObj.created_tasks) {
        $bulkA = $resultObj.created_tasks | Where-Object { $_.name -eq 'WF Bulk Implicit A' } | Select-Object -First 1
        $bulkB = $resultObj.created_tasks | Where-Object { $_.name -eq 'WF Bulk Explicit B' } | Select-Object -First 1
        if ($bulkA) {
            $jsonA = Get-Content $bulkA.file_path -Raw | ConvertFrom-Json
            Assert-Equal -Name "task_create_bulk injects workflow when missing" `
                -Expected 'test-workflow' `
                -Actual $jsonA.workflow
        }
        if ($bulkB) {
            $jsonB = Get-Content $bulkB.file_path -Raw | ConvertFrom-Json
            Assert-Equal -Name "task_create_bulk preserves per-task explicit workflow" `
                -Expected 'per-task-override' `
                -Actual $jsonB.workflow
        }
    }
} catch {
    Write-TestResult -Name "Workflow attribution tests" -Status Fail -Message "Exception: $($_.Exception.Message)"
} finally {
    if ($wfMcpProcess) {
        Stop-McpServer -Process $wfMcpProcess
    }
    # Restore (or clear) the env var so subsequent tests aren't affected.
    $env:DOTBOT_CURRENT_WORKFLOW = $savedWorkflowEnv
}
